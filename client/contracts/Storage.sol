...
contract Storage {
    ...
    function createDiplome(string[] memory _dip) public {
        ...
        require(_dip.length == 11, "Error some parameters missing. Should be at least 11 parameters");
        for (uint i = 0; i < diplomes.length; i++) {
            if ( (keccak256(bytes((diplomes[i].CNE))) == keccak256(bytes((_dip[0])))) 
                && (keccak256(bytes((diplomes[i].Diplome))) == keccak256(bytes((_dip[8])))) 
                && (keccak256(bytes((diplomes[i].Date))) == keccak256(bytes((_dip[10])))) ){
               return;
            }
        }
    ...
    function filterDiplomes(string memory _date, string memory _dip, string memory _etab) external view returns(Diplome[] memory) {
        ...
        for (uint i = 0; i < diplomes.length; i++) {
            if ((keccak256(abi.encodePacked((diplomes[i].Date))) == keccak256(abi.encodePacked((_date)))) 
            && (keccak256(abi.encodePacked((diplomes[i].Diplome))) == keccak256(abi.encodePacked((_dip)))) 
            && (keccak256(abi.encodePacked((diplomes[i].faculty))) == keccak256(abi.encodePacked((_etab))))) {
                resultCount++; 
            }
        }
    ...
    function getDiplome(string memory _CNE, string memory _dip, string memory _date) public view returns (Diplome memory) {
        for(uint i = 0; i < diplomes.length; i++){
            if ( (keccak256(bytes(diplomes[i].CNE)) == keccak256(bytes(_CNE))) 
            && (keccak256(bytes(diplomes[i].Diplome)) == keccak256(bytes(_dip)))
            && (keccak256(bytes(diplomes[i].Date)) == keccak256(bytes(_date))) ){
                return diplomes[i];
            }
        }
    }
